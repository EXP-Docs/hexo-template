...
#-----------------------------------------------------------------
# 命令执行示例：
# bin/init.sh -e "289065406@qq.com" -u "lyy289065406" -d "articles"
#-----------------------------------------------------------------
...
bin/_sed.sh "^url: .*$" "url: https:\/\/${BLOG_DOMAIN}" "${HEXO_CONFIG}"
bin/_sed.sh "^  repo: .*$" "  repo: git@github.com:${GITHUB_USER}\/${DEPLOY_REPO_NAME}.git" "${HEXO_CONFIG}"
echo ${BLOG_DOMAIN} > ./hexo/source/CNAME

# 根据 package.json 安装 hexo 插件
echo "Install npm plugins ..."
bin/exec.sh npm install
