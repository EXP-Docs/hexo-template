...
if [[ ${ENV} != "prod" ]] ; then
    ...
    bin/_sed.sh "^url: .*$" "url: ${GITHUB_DOMAIN//\//\\/}" "${HEXO_CONFIG}"
    bin/_sed.sh "^root: .*$" "root: \/${CUR_REPO_NAME}\/hexo\/public\/" "${HEXO_CONFIG}"

# 设置生产环境参数
else
    DEPLOY_REPO_NAME=`cat ${HEXO_CONFIG} | grep "  repo: " | awk -F '[./]' '{print $3}'`
    bin/_sed.sh "^url: .*$" "url: ${CUSTOM_DOMAIN//\//\\/}" "${HEXO_CONFIG}"

    # Github 子域名
    if [[ ${USEDOMAIN} = "no" ]] ; then
        bin/_sed.sh "^root: .*$" "root: \/${DEPLOY_REPO_NAME}\/" "${HEXO_CONFIG}"

    # 自定义域名
    else
        bin/_sed.sh "^root: .*$" "root: \/" "${HEXO_CONFIG}"
    fi
fi

# 清除历史内容
bin/exec.sh hexo clean
