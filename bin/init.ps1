...
#-----------------------------------------------------------------
# 命令执行示例：
# bin/init.ps1 -e "289065406@qq.com" -u "lyy289065406" -d "articles"
#-----------------------------------------------------------------
...
function usage {
    ...
    Write-Host "-n <deploy repo name> The repo (Not this one) for deploying hexo-template's articles"
...
bin/_sed.ps1 "^url: .*$" "url: https://${BLOG_DOMAIN}" "${HEXO_CONFIG}"
bin/_sed.ps1 "^  repo: .*$" "  repo: git@github.com:${GITHUB_USER}/${DEPLOY_REPO_NAME}.git" "${HEXO_CONFIG}"
Set-Content -Path ./hexo/source/CNAME -Value ${BLOG_DOMAIN}


# 根据 package.json 安装 hexo 插件
Write-Host "Install npm plugins ..."
bin/exec.ps1 npm install
