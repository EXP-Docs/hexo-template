...
if (!(${ENV} -Eq "prod")) {
    ...
    ./_sed.ps1 "^url: .*$" "url: ${GITHUB_DOMAIN}" "${HEXO_CONFIG}"
    ./_sed.ps1 "^root: .*$" "root: /${CUR_REPO_NAME}/hexo/public/" "${HEXO_CONFIG}"

# 设置生产环境参数
} else {
    $DEPLOY_REPO_NAME = ((Get-Content ${HEXO_CONFIG} -encoding utf8 | Select-String -pattern "  repo: ") -Split "[/\.]")[2]
    ./_sed.ps1 "^url: .*$" "url: ${CUSTOM_DOMAIN}" "${HEXO_CONFIG}"

    # Github 子域名
    if (${USEDOMAIN} -Eq "no") {
        ./_sed.ps1 "^root: .*$" "root: /${DEPLOY_REPO_NAME}/hexo/public/" "${HEXO_CONFIG}"

    # 自定义域名
    } else {
        ./_sed.ps1 "^root: .*$" "root: /" "${HEXO_CONFIG}"
    }
