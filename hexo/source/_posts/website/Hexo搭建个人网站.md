...
另模板仓库所提供脚本适用于 Linux/Windows/Mac 三端，故下文中提及的 `*.sh` 均有配套的 `*.ps1` 脚本，以应对在不同环境下的使用，且使用方法均是一致的。
...
该命令会在 [`hexo/source/_posts/`](https://github.com/lyy289065406/hexo-template/tree/master/hexo/source/_posts) 目录下创建 2 个文件：
...
也可以直接在 [`hexo/source/_posts/`](https://github.com/lyy289065406/hexo-template/tree/master/hexo/source/_posts) 目录下创建 markdown 文件和同名目录，效果是一样的。
...
该命令会根据 [`hexo/source/`](hexo/source) 目录下的 markdown 文件在 [`hexo/public/`](https://github.com/lyy289065406/hexo-template/tree/master/hexo/public) 目录下生成 html 格式的站点内容。
